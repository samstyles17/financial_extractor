...
from file_extractor import image_to_text, pdf_to_jpg, excelToImage, xlstoxlsx
...
def ratios(response):
    ...
    try:
        dataframeratios['quickratio_exinven'] = (dataframe['Current Assets'] - dataframe['Inventories']) / dataframe[
    ...
    try:
        dataframeratios['cash_toworkcap'] = dataframe['Cash'] / (
                dataframe['Current Assets'] - dataframe['Total Current Liabilities'])
    except:
    ...
    try:
        dataframeratios['workingcap_byassets'] = (dataframe['Current Assets'] - dataframe[
    ...
    final_op = dataframeratios.to_json(orient='index')

    dataframe.to_excel('zomato.xlsx')

    with open('zomato_ratios.json', 'w') as json_file:
        json_file.write(final_op)

    print(dataframeratios)
...
print(img)
print(type(img))
your_json = image_to_text(path_ocr,img)
print(your_json)
with open('zomato_op2.json', 'w') as json_file:
    json.dump(your_json, json_file)

import numpy as np

# your_json2 = {}
# print("Number of channels:", img[69:70].shape)
# income_statement = np.asarray(img[69:70])
# cash_flow_statement = np.asarray(img[72:73])
# balance_sheet = np.asarray(img[68:69])
# json_income = image_to_text(path_ocr, income_statement)
# json_balance = image_to_text(path_ocr, balance_sheet)
# json_cashflow = image_to_text(path_ocr, cash_flow_statement)


#
# print()
# with open('list_output_blackrock_new.json', 'w') as json_file:
#     json.dump(your_json, json_file)


# your_json = {}
# img_paths = {
#     "income_statement": img[69:70],
#     "cash_flow_statement": img[72:73],
#     "balance_sheet": img[68:69]
# }
#
# result_list = []
# for statement_type, img_slice in img_paths.items():
#     result = image_to_text(path_ocr, {statement_type: [img_slice]})
#     result_list.append(result)
#
# # Save as JSON
# with open('blackrock_all.json', 'w') as json_file:
...
"""excel_file = "AMZN-10k.xls"
ext = os.path.splitext(excel_file)[1]
if ext == ".xls":
    modified_excel = "modified.xlsx"
    xlstoxlsx(excel_file, modified_excel)
    new_xl_img = excelToImage(modified_excel)
    new_xl_json = image_to_text(path_ocr, new_xl_img)
    print(new_xl_json)
    with open('xl_output4.json', 'w') as json_file:
        json.dump(new_xl_json, json_file)
    ratios(new_xl_json)

else:
    xl_img = excelToImage(excel_file)
    xl_json = image_to_text(path_ocr, xl_img)
    print(xl_json)
    with open('xl_output4.json', 'w') as json_file:
        json.dump(xl_json, json_file)

    ratios(xl_json)"""
"""
    1. If total equity is not available, equity share capital+other equity 
    2. If total tax is not there , total tax = deferred tax + current tax
    3. Non -current investments -> the word investment is common to both current and non-current investments
    4. NC Liabilities = Financial Liabilities + Provisions + Deferred tax Liabilities + Other non-current liabilities
    5. Earnings per share = Basic + Diluted
    6. Receivables and payables only from current assets and current liabilities
"""

"""
l1 = [img[68:69],img[69:70],img[72:73]]
result1 = {}
for images in l1:
    result = image_to_text(ocr_path, images)
